...
mod tests {
    ...
    #[test]
    fn build_timing_xml_empty() {
        assert_eq!(build_timing_xml(&[]), "");
    }

    #[test]
    fn build_timing_xml_fade_animation() {
        let anims = vec![
            Anim::Fade { sid: 2, dur: 500, delay: 0, trigger: Trigger::OnClick },
        ];
        let xml = build_timing_xml(&anims);
        assert!(xml.contains("<p:timing>"), "should contain timing element");
        assert!(xml.contains("<p:anim"), "should contain anim element");
        assert!(xml.contains("style.opacity"), "should animate opacity");
        assert!(xml.contains("<p:seq"), "should have sequence container");
    }

    #[test]
    fn build_transition_xml_fade() {
        let te = TransitionEffect {
            transition_type: "fade".to_string(),
            direction: String::new(),
            color: None,
        };
        let xml = build_transition_xml(&te, "med");
        assert!(xml.contains("<p:transition"));
        assert!(xml.contains("<p:fade"));
        assert!(xml.contains("spd=\"med\""));
    }

    #[test]
    fn build_transition_xml_push() {
        let te = TransitionEffect {
            transition_type: "push".to_string(),
            direction: "r".to_string(),
            color: None,
        };
        let xml = build_transition_xml(&te, "fast");
        assert!(xml.contains("<p:push"));
        assert!(xml.contains("dir=\"r\""));
        assert!(xml.contains("spd=\"fast\""));
    }

    #[test]
    fn build_transition_xml_zoom() {
        let te = TransitionEffect {
            transition_type: "zoom".to_string(),
            direction: String::new(),
            color: None,
        };
        let xml = build_transition_xml(&te, "slow");
        assert!(xml.contains("<p:zoom"));
        assert!(xml.contains("spd=\"slow\""));
    }

    #[test]
    fn inject_animations_adds_timing() {
...
<p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
  <p:cSld><p:spTree/></p:cSld>
        ...
        let anims = vec![
            Anim::Fade { sid: 2, dur: 300, delay: 0, trigger: Trigger::OnClick },
        ];
        let result = inject_animations(slide_xml, &anims, None, "med").unwrap();
        let s = String::from_utf8_lossy(&result);
        assert!(s.contains("<p:timing>"), "should inject timing: {}", s);
        assert!(s.contains("style.opacity"), "should contain opacity animation");
    }

    #[test]
    fn inject_animations_adds_transition() {
        let slide_xml = r#"<?xml version="1.0" encoding="UTF-8"?>
<p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
  <p:cSld><p:spTree/></p:cSld>
</p:sld>"#.as_bytes();
        ...
        let result = inject_animations(slide_xml, &[], Some(&te), "med").unwrap();
        let s = String::from_utf8_lossy(&result);
        assert!(s.contains("<p:transition"), "should inject transition: {}", s);
        assert!(s.contains("<p:fade"), "should contain fade transition");
    }

    #[test]
    fn inject_animations_removes_existing_timing() {
        let slide_xml = r#"<?xml version="1.0" encoding="UTF-8"?>
<p:sld xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
  <p:cSld><p:spTree/></p:cSld>
  <p:timing><p:tnLst><p:par/></p:tnLst></p:timing>
        ...
        let result = inject_animations(slide_xml, &anims, None, "med").unwrap();
        let s = String::from_utf8_lossy(&result);
        // Old timing should be gone, new one should be present
        let timing_count = s.matches("<p:timing>").count();
        assert_eq!(timing_count, 1, "should have exactly one timing element: {}", s);
    }

    #[test]
    fn parse_svg_animate_tag_opacity() {
        let attrs = BTreeMap::from([
            ("attributeName".to_string(), "opacity".to_string()),
            ("from".to_string(), "0".to_string()),
            ("to".to_string(), "1".to_string()),
            ("dur".to_string(), "1s".to_string()),
        ]);
        let anim = parse_svg_anim_tag(&attrs);
        assert!(anim.is_some());
        let a = anim.unwrap();
        assert_eq!(a.attr, "opacity");
        assert_eq!(a.from, Some("0".to_string()));
        assert_eq!(a.to, "1");
        assert_eq!(a.dur_ms, Some(1000));
    }

    #[test]
    fn svg_anim_to_ooxml_fade_entrance() {
        let svg = SvgAnim {
            target_id: None,
            attr: "opacity".to_string(),
            from: Some("0".to_string()),
            to: "1".to_string(),
            dur_ms: Some(500),
            begin_ms: Some(0),
        };
        let anim = svg_anim_to_ooxml(&svg, 2);
        assert!(anim.is_some());
        match anim.unwrap() {
            Anim::Fade { sid: 2, dur: 500, delay: 0, trigger: Trigger::OnClick } => {}
    ...
    #[test]
    fn svg_anim_to_ooxml_fade_exit() {
        let svg = SvgAnim {
            target_id: None,
            attr: "opacity".to_string(),
        ...
        let anim = svg_anim_to_ooxml(&svg, 3);
        assert!(anim.is_some());
        match anim.unwrap() {
            Anim::ExitFade { sid: 3, dur: 300, delay: 0, trigger: Trigger::OnClick } => {}
            other => panic!("expected ExitFade, got {:?}", other),
        }
    }

    #[test]
    fn resolve_shape_ids_all() {
        let spec = AnimationSpec {
            shape: Some("@all".to_string()),
        ...
        let ids = resolve_shape_ids(&spec, 5);
        assert_eq!(ids, vec![2, 3, 4, 5, 6]); // 5 shapes → ids 2..=6
    }

    #[test]
    fn resolve_shape_ids_first() {
        let spec = AnimationSpec {
            shape: Some("@first".to_string()),
            effect: "fadeIn".to_string(),
            duration_ms: 500,
            delay_ms: 0,
            trigger: "onclick".to_string(),
    ...
    #[test]
    fn resolve_shape_ids_index() {
        let spec = AnimationSpec {
            shape: Some("1".to_string()),
            effect: "flyIn".to_string(),
            duration_ms: 500,
            delay_ms: 0,
            trigger: "onclick".to_string(),
            direction: "r".to_string(),
        ...
        let ids = resolve_shape_ids(&spec, 5);
        assert_eq!(ids, vec![3]); // index 1 → shape_id 3 (2 + 1)
    }

    #[test]
    fn resolve_shape_ids_out_of_bounds() {
        let spec = AnimationSpec {
            shape: Some("99".to_string()),
            effect: "fadeIn".to_string(),
            duration_ms: 500,
            delay_ms: 0,
            trigger: "onclick".to_string(),
    ...
    #[test]
    fn spec_to_anim_fly_in() {
        let spec = AnimationSpec {
            shape: None,
            effect: "flyIn".to_string(),
            duration_ms: 400,
            delay_ms: 100,
            trigger: "afterprev".to_string(),
            direction: "r".to_string(),
            zoom_scale: None,
        };
        let anim = spec_to_anim(&spec, 4);
        match anim {
            Anim::FlyIn { sid: 4, dur: 400, delay: 100, trigger: Trigger::AfterPrev, dir: FlyDir::R } => {}
            other => panic!("expected FlyIn, got {:?}", other),
        }
    }

    #[test]
    fn spec_to_anim_zoom() {
        let spec = AnimationSpec {
            shape: None,
            effect: "zoom".to_string(),
            duration_ms: 600,
            delay_ms: 0,
            trigger: "onclick".to_string(),
            direction: String::new(),
        ...
        let anim = spec_to_anim(&spec, 5);
        match anim {
            Anim::Zoom { sid: 5, dur: 600, delay: 0, trigger: Trigger::OnClick, from_scale: 0.0 } => {}
            other => panic!("expected Zoom, got {:?}", other),
        }
    }

    #[test]
    fn spec_to_anim_exit_fade_out() {
        let spec = AnimationSpec {
            shape: None,
            effect: "fadeOut".to_string(),
            duration_ms: 300,
            delay_ms: 0,
            trigger: "onclick".to_string(),
            direction: String::new(),
        ...
        let anim = spec_to_anim(&spec, 2);
        match anim {
            Anim::ExitFade { sid: 2, dur: 300, delay: 0, trigger: Trigger::OnClick } => {}
            other => panic!("expected ExitFade, got {:?}", other),
        }
    }

    #[test]
    fn build_timing_xml_multiple_animations() {
        let anims = vec![
            Anim::Fade { sid: 2, dur: 300, delay: 0, trigger: Trigger::OnClick },
            Anim::FlyIn { sid: 3, dur: 400, delay: 0, trigger: Trigger::AfterPrev, dir: FlyDir::L },
            Anim::ExitFade { sid: 4, dur: 200, delay: 0, trigger: Trigger::WithPrev },
        ];
        let xml = build_timing_xml(&anims);
        assert!(xml.contains("<p:timing>"));
        assert!(xml.contains("style.opacity"));
        assert!(xml.contains("ppt_x"));
    }

    #[test]
    fn build_timing_xml_with_delay() {
        let anims = vec![
            Anim::Fade { sid: 2, dur: 500, delay: 1000, trigger: Trigger::OnClick },
        ];
        let xml = build_timing_xml(&anims);
        assert!(xml.contains("delay=\"1000\""), "should contain delay: {}", xml);
    }

    #[test]
    fn extract_slide_index_various() {
        assert_eq!(extract_slide_index("ppt/slides/slide1.xml"), 1);
        assert_eq!(extract_slide_index("ppt/slides/slide10.xml"), 10);
        assert_eq!(extract_slide_index("ppt/slides/slide99.xml"), 99);
        assert_eq!(extract_slide_index("foo/slide3.xml"), 3);
        assert_eq!(extract_slide_index("invalid"), 0);
    }
...
<p:sld>
  <p:cSld><p:spTree>
    ...
    <p:cxnSp><p:nvCxnSpPr/></p:cxnSp>
        ...
        assert_eq!(count_shapes(xml), 3);
    }

    #[test]
    fn fly_dir_from_str() {
        assert!(matches!(FlyDir::from_str("l"), FlyDir::L));
        assert!(matches!(FlyDir::from_str("R"), FlyDir::R));
        assert!(matches!(FlyDir::from_str("t"), FlyDir::T));
        assert!(matches!(FlyDir::from_str("bottom"), FlyDir::B));
        assert!(matches!(FlyDir::from_str("tl"), FlyDir::TL));
        assert!(matches!(FlyDir::from_str("TOPRIGHT"), FlyDir::TR));
        assert!(matches!(FlyDir::from_str("unknown"), FlyDir::L)); // default
    }

    #[test]
    fn emphasis_style_from_effect() {
        assert!(matches!(EmphasisStyle::from_effect("spin"), EmphasisStyle::Spin));
        assert!(matches!(EmphasisStyle::from_effect("pulse"), EmphasisStyle::Pulse));
        assert!(matches!(EmphasisStyle::from_effect("growTurn"), EmphasisStyle::GrowTurn));
        assert!(matches!(EmphasisStyle::from_effect("unknown"), EmphasisStyle::Pulse)); // default
    }
}

// Re-exports
pub use CreatePptxAnimationTool as CreatePptxAnimationToolImpl;
pub use AddAnimationTool as AddAnimationToolImpl;
