  ...
  <w:body>
    <w:p>
      ...
      <w:r>
        ...
        let (paragraphs, _image_markers, _sections) = parse_document_xml(src).expect("parse should succeed");
  ...
  <w:body>
    <w:tbl>
      ...
      <w:tr>
        ...
        let doc = WordDocument { paragraphs, tables: tbls, images: vec![], sections: vec![], headers: vec![], footers: vec![] };
        ...
        let rebuilt_doc = WordDocument { paragraphs: vec![], tables: vec![rebuilt.tables.into_iter().next().unwrap()], images: vec![], sections: vec![], headers: vec![], footers: vec![] };
    ...
    fn test_multiple_tables_inline_positions() {
        let elements = vec![
            DocElement::Paragraph { id: "p1".to_string(), text: "P1".to_string(), omit_text: false, style: None, runs: None, numbering: None, alignment: None, text_direction: None },
            DocElement::Table { id: "t0".to_string(), position: 0, header: vec![TableCell::plain("T1".to_string())], rows: vec![] },
            DocElement::Paragraph { id: "p2".to_string(), text: "P2".to_string(), omit_text: false, style: None, runs: None, numbering: None, alignment: None, text_direction: None },
            DocElement::Table { id: "t1".to_string(), position: 0, header: vec![TableCell::plain("T2".to_string())], rows: vec![] },
            DocElement::Paragraph { id: "p3".to_string(), text: "P3".to_string(), omit_text: false, style: None, runs: None, numbering: None, alignment: None, text_direction: None },
        ];
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  <Relationship Id="rId9" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/image" Target="media/image7.png"/>
    ...
    // ─── Section / header / footer / field-code regression tests ─────────

    /// A single-section document with vertical text direction must round-trip
    /// through the XML build → parse cycle. Specifically, the parsed section
    /// must carry `text_direction = "verticalRightToLeft"`.
    #[test]
    fn roundtrip_vertical_text_direction() {
        let doc = WordDocument {
            paragraphs: vec![WordParagraph {
                id: "p0".to_string(),
                text: "古诗".to_string(),
        ...
        let mut buf = std::io::Cursor::new(Vec::<u8>::new());
        write_word_document(&doc, &mut buf, None).expect("write");
        let reread = read_word_document(&buf.into_inner()).expect("read");
        assert_eq!(reread.sections.len(), 1, "should keep one section");
        assert_eq!(
            reread.sections[0].text_direction.as_deref(),
            Some("verticalRightToLeft"),
            "vertical text direction must survive round-trip"
        );
        let p = &reread.paragraphs[0];
        assert_eq!(
            p.text_direction.as_deref(),
            Some("verticalRightToLeft"),
            "vertical paragraph direction must survive round-trip"
        );
        assert_eq!(p.text, "古诗");
    }

    /// A multi-section document (landscape cover + portrait body) must
    /// round-trip with both sections and their distinct page sizes.
    #[test]
    fn roundtrip_multi_section_landscape_then_portrait() {
        let doc = WordDocument {
            paragraphs: vec![WordParagraph {
                id: "p0".to_string(),
        ...
        write_word_document(&doc, &mut buf, None).expect("write");
        let reread = read_word_document(&buf.into_inner()).expect("read");
        assert_eq!(reread.sections.len(), 2, "both sections should be preserved");
        // The cover section's orient should be landscape.
        let cover = &reread.sections[0];
        let cover_size = cover.page_size_mm.as_ref().expect("cover size should be present");
        assert_eq!(cover_size.orient.as_deref(), Some("landscape"));
        // The body section's orient should be portrait.
        let body = &reread.sections[1];
        let body_size = body.page_size_mm.as_ref().expect("body size should be present");
        assert_eq!(body_size.orient.as_deref(), Some("portrait"));
    }

    /// A document with a header part and a footer part referenced from
    /// its single section must:
    ///   1. produce a `.docx` containing `word/header1.xml` and `word/footer1.xml`
    ///   2. wire up rIds in `word/_rels/document.xml.rels`
    ///   3. add the matching `<Override>` entries in `[Content_Types].xml`
    ///   4. embed `<w:headerReference>` / `<w:footerReference>` in the sectPr
    #[test]
    fn roundtrip_header_footer_with_section_refs() {
        let doc = WordDocument {
            paragraphs: vec![WordParagraph {
                id: "p0".to_string(),
                text: "Body".to_string(),
            ...
            sections: vec![WordSection {
                id: "s1".to_string(),
                header_refs: vec![HeaderPartRef {
                    header_id: "h1".to_string(),
                    kind: Some("default".to_string()),
                }],
                footer_refs: vec![FooterPartRef {
        ...
        // The zip should contain header1.xml / footer1.xml.
        let mut archive = zip::ZipArchive::new(std::io::Cursor::new(&bytes[..])).expect("zip");
        ...
        assert!(
            ...
            "expected word/header1.xml; got {:?}",
            names
        );
        assert!(
            names.iter().any(|n| n == "word/footer1.xml"),
            "expected word/footer1.xml; got {:?}",
            names
        );

        // document.xml.rels must include both header and footer relationships.
        let mut archive = zip::ZipArchive::new(std::io::Cursor::new(&bytes[..])).expect("zip");
        ...
        for i in 0..archive.len() {
            ...
            if f.name() == "word/_rels/document.xml.rels" {
                use std::io::Read;
                let _ = f.read_to_string(&mut rels_xml);
            }
        }
        assert!(
            rels_xml.contains("header1.xml") && rels_xml.contains("http://schemas.openxmlformats.org/officeDocument/2006/relationships/header"),
            "rels should reference header1.xml with the header relationship type; got: {}",
            rels_xml
        );
        assert!(
            rels_xml.contains("footer1.xml") && rels_xml.contains("http://schemas.openxmlformats.org/officeDocument/2006/relationships/footer"),
            "rels should reference footer1.xml with the footer relationship type; got: {}",
            rels_xml
        );

        // [Content_Types].xml must include header/footer Overrides.
        let mut archive = zip::ZipArchive::new(std::io::Cursor::new(&bytes[..])).expect("zip");
        ...
        for i in 0..archive.len() {
            ...
            if f.name() == "[Content_Types].xml" {
                use std::io::Read;
                let _ = f.read_to_string(&mut ct_xml);
            }
        }
        assert!(
            ct_xml.contains("header+xml"),
            "Content_Types should include header content type; got: {}",
            ct_xml
        );
        assert!(
            ct_xml.contains("footer+xml"),
            "Content_Types should include footer content type; got: {}",
            ct_xml
        );
        // The Override for each header/footer part MUST appear exactly
        // once. A duplicate `PartName` makes Word/WPS reject the package
        // (or silently drop one of the references). The historical bug
        // was: `CONTENT_TYPES_XML` shipped a hardcoded `<Override
        // PartName="/word/header1.xml" .../>` AND `append_hf_overrides`
        // appended one too, producing two entries with the same
        // PartName.
        let header_override_count = ct_xml.matches("/word/header1.xml").count();
        assert_eq!(
            header_override_count, 1,
            "header1.xml Override must appear exactly once; got {} occurrences in: {}",
            header_override_count, ct_xml
        );
        let footer_override_count = ct_xml.matches("/word/footer1.xml").count();
        assert_eq!(
            footer_override_count, 1,
            "footer1.xml Override must appear exactly once; got {} occurrences in: {}",
            footer_override_count, ct_xml
        );

        // The document.xml must include a headerReference and footerReference in sectPr.
        let mut archive = zip::ZipArchive::new(std::io::Cursor::new(&bytes[..])).expect("zip");
        let mut doc_xml = String::new();
        for i in 0..archive.len() {
            let mut f = archive.by_index(i).unwrap();
            if f.name() == "word/document.xml" {
                use std::io::Read;
                let _ = f.read_to_string(&mut doc_xml);
            }
        }
        assert!(
            doc_xml.contains("<w:headerReference"),
            "document.xml must embed <w:headerReference> in sectPr; got: {}",
            doc_xml
        );
        assert!(
            doc_xml.contains("<w:footerReference"),
            "document.xml must embed <w:footerReference> in sectPr; got: {}",
            doc_xml
        );
        // The placeholder rId from `build_sectpr_xml` (e.g. `rIdHeaderPlaceholder_h1`)
        // must have been substituted to a real `rId<N>` form, otherwise Word
        // and the rels file see different ids and the header/footer is
        // effectively dropped. This is the bug that bit "页眉页脚好像没有生效".
        assert!(
            !doc_xml.contains("Placeholder"),
            "header/footer placeholder rIds were not substituted; got: {}",
            doc_xml
        );
        // The actual rId appearing inside headerReference should match one of
        // the rIds declared in document.xml.rels (so the rels lookup actually
        // resolves to header1.xml / footer1.xml).
        let mut archive = zip::ZipArchive::new(std::io::Cursor::new(&bytes[..])).expect("zip");
        let mut rels_xml2 = String::new();
        for i in 0..archive.len() {
            let mut f = archive.by_index(i).unwrap();
            if f.name() == "word/_rels/document.xml.rels" {
                use std::io::Read;
                let _ = f.read_to_string(&mut rels_xml2);
            }
        }
        let header_rid_in_doc = {
            let start = doc_xml.find("r:id=\"").expect("header ref exists");
            let start = start + "r:id=\"".len();
            let end = doc_xml[start..].find('"').expect("rId is quoted");
            doc_xml[start..start + end].to_string()
        };
        assert!(
            rels_xml2.contains(&format!("Id=\"{}\"", header_rid_in_doc)),
            "header rId `{}` from document.xml is not declared in document.xml.rels",
            header_rid_in_doc
        );
    }

    /// Regression test for the body-level `<w:sectPr>` read-back bug.
    /// Before the fix the parser only recognised `<w:sectPr>` when it
    /// appeared inside a `<w:pPr>` (an in-paragraph section break).
    /// The trailing `<w:sectPr>` at the end of `<w:body>` was silently
    /// dropped, which meant a doc that round-trips through
    /// `read_word_document` ended up with `sections: []` even though
    /// `document.xml` had a perfectly valid body-level sectPr.
    ///
    /// Combined with the Empty-event handling fix below, the section's
    /// `header_refs` / `footer_refs` must also survive the round-trip —
    /// the writer emits `<w:headerReference .../>` as a self-closing
    /// tag, which `quick_xml` surfaces as an `Empty` event rather than
    /// `Start`+`End`.
    #[test]
    fn roundtrip_body_sectpr_picks_up_hdr_ftr_refs() {
        let original = WordDocument {
            paragraphs: vec![WordParagraph {
                id: "p0".to_string(),
                text: "Body".to_string(),
                ..Default::default()
        ...
        let mut buf = std::io::Cursor::new(Vec::<u8>::new());
        write_word_document(&original, &mut buf, None).expect("write");
        let bytes = buf.into_inner();

        let parsed = read_word_document(&bytes).expect("parse back");

        assert_eq!(parsed.headers.len(), 2, "should keep both header parts");
        assert_eq!(parsed.footers.len(), 1, "should keep the footer part");
        assert_eq!(parsed.sections.len(), 1, "body-level sectPr must be parsed");
        let sect = &parsed.sections[0];
        assert_eq!(
            sect.header_refs.len(),
            2,
            "both header refs must survive (default + first)"
        );
        assert_eq!(
            sect.footer_refs.len(),
            1,
            "the footer ref must survive read-back"
        );
        // rId resolution: header_id must be the file basename so the
        // writer can look up the matching part on the next save.
        let default_hdr = sect
            .header_refs
            .iter()
            .find(|r| r.kind.as_deref() == Some("default"))
            .expect("default header ref");
        assert_eq!(default_hdr.header_id, "header1");
        let first_hdr = sect
            .header_refs
            .iter()
            .find(|r| r.kind.as_deref() == Some("first"))
            .expect("first header ref");
        assert_eq!(first_hdr.header_id, "header2");
        assert_eq!(sect.footer_refs[0].footer_id, "footer1");
    }

    /// A `FontRun` carrying a `FieldRef::Page` must produce a fldChar
    /// triplet in the built XML — i.e. the writer must convert it to the
    /// OOXML field-code form, NOT a plain text run.
    #[test]
    fn build_run_xml_with_page_field() {
        let run = FontRun {
            text: "".to_string(),
        ...
        assert!(
            ...
            "fldChar missing from field run XML: {}",
            xml
        );
        assert!(
            xml.contains("PAGE") || xml.contains("PAGE "),
            "PAGE instruction missing from field run XML: {}",
            xml
        );
    }

    /// A `FontRun` carrying a `FieldRef::NumPages` must produce a fldChar
    /// triplet with the NUMPAGES instruction.
    #[test]
    fn build_run_xml_with_numpages_field() {
        let run = FontRun {
            text: "".to_string(),
        ...
        let xml = build_run_xml(&run);
        assert!(xml.contains("w:fldChar"), "fldChar missing: {}", xml);
        assert!(
            xml.contains("NUMPAGES"),
            "NUMPAGES instruction missing: {}",
            xml
        );
    }

    /// A `FontRun` carrying a custom `FieldRef` must round-trip its raw
    /// `instr` text.
    #[test]
    fn build_run_xml_with_custom_field() {
        let run = FontRun {
            text: "".to_string(),
            field: Some(FieldRef::Custom {
                instr: "DOCPROPERTY Company".to_string(),
            }),
            ..Default::default()
        };
        let xml = build_run_xml(&run);
        assert!(xml.contains("w:fldChar"), "fldChar missing: {}", xml);
        assert!(
            xml.contains("DOCPROPERTY Company"),
            "custom instr text missing: {}",
            xml
        );
    }

    /// A `FontRun` with `vert_align = "superscript"` must emit the
    /// `<w:vertAlign w:val="superscript"/>` element.
    #[test]
    fn build_run_xml_with_vert_align() {
        let run = FontRun {
            text: "x".to_string(),
    ...
    /// `preserve_from` (modifying an existing file) must not lose
    /// previously-defined header / footer relationships from the
    /// `document.xml.rels` — they should still be in the rels file
    /// after the round-trip.
    #[test]
    fn preserve_existing_header_rels() {
        // Build an initial doc with a header part.
        ...
        let mut buf = std::io::Cursor::new(Vec::<u8>::new());
        write_word_document(&initial, &mut buf, None).expect("write initial");
        let initial_bytes = buf.into_inner();

        // Now "modify" the same doc by re-saving the same doc with
        // `preserve_from` = the original bytes. The header rels should
        // survive the round-trip.
        let mut buf2 = std::io::Cursor::new(Vec::<u8>::new());
        write_word_document(&initial, &mut buf2, Some(&initial_bytes)).expect("write preserve");
        let preserved_bytes = buf2.into_inner();

        let mut archive = zip::ZipArchive::new(std::io::Cursor::new(&preserved_bytes[..])).expect("zip");
        let mut names: Vec<String> = (0..archive.len())
            .map(|i| archive.by_index(i).unwrap().name().to_string())
            .collect();
        names.sort();
        assert!(
            names.iter().any(|n| n == "word/header1.xml"),
            "header1.xml should still exist after preserve; got {:?}",
            names
        );

        // Header rels should also be preserved across the round-trip. If
        // either the rels entry or the doc.xml headerReference disappeared,
        // opening the resulting .docx in Word shows no header at all —
        // this is exactly the failure that drove "页眉页脚好像没有生效".
        let mut rels_xml = String::new();
        let mut doc_xml = String::new();
        let mut archive = zip::ZipArchive::new(std::io::Cursor::new(&preserved_bytes[..])).expect("zip");
        for i in 0..archive.len() {
        ...
        assert!(
            rels_xml.contains("http://schemas.openxmlformats.org/officeDocument/2006/relationships/header"),
            "preserved rels lost the header relationship: {}",
            rels_xml
        );
        assert!(
            doc_xml.contains("<w:headerReference") && !doc_xml.contains("Placeholder"),
            "preserved document.xml lost the headerReference or kept a placeholder: {}",
            doc_xml
        );
    }

    /// The build_paragraph_ppr_xml must include `<w:jc>` when alignment is
    /// set, and `<w:textDirection>` when text_direction is set.
    #[test]
    fn build_paragraph_ppr_includes_alignment_and_direction() {
        let para = WordParagraph {
            id: "p0".to_string(),
            text: "x".to_string(),
        ...
        let xml = build_paragraph_ppr_xml(&para, None);
        assert!(xml.contains("<w:jc w:val=\"center\"/>"), "missing jc: {}", xml);
        assert!(
            xml.contains("<w:textDirection w:val=\"verticalRightToLeft\"/>"),
            "missing textDirection: {}",
            xml
        );
    }

    /// The body-level sectPr must include page size, margins, and
    /// text direction when set on the section.
    #[test]
    fn build_body_sectpr_includes_full_layout() {
        let sect = WordSection {
            id: "s1".to_string(),
            page_size_mm: Some(PageSizeMm {
                width: 210.0,
                height: 297.0,
        ...
        let xml = build_body_sectpr_xml(&sect);
        assert!(xml.contains("<w:pgSz"), "missing pgSz: {}", xml);
        assert!(xml.contains("<w:pgMar"), "missing pgMar: {}", xml);
        assert!(
            xml.contains("<w:textDirection"),
            "missing textDirection: {}",
            xml
        );
    }
