...
**注意事项**：
- `read_image` 读取并登记的单张文件最大 20 MB。更大的图片必须先压缩或缩放。
- `asset_id` 在进程内有效 1 小时；超过后再次引用会得到清晰的 `unknown or expired` 错误提示，此时重新调用 `read_image` 即可。
- 当所选模型支持视觉时，调用 `read_image` 后，系统会自动把这张图片的像素附加到**下一轮**模型请求；因此可以直接检查截图内容、布局、遮挡、溢出和视觉层级，无需把 base64 写入回答或再调用其他工具。要在后续轮次重新检查修改结果，需对新的截图再次调用 `read_image`。
- 视觉检查当前仅支持 `png`、`jpg/jpeg`、`webp`、`gif`，且每张最多 12 MB、一次最多 8 张、总计最多 32 MB。`bmp`、`ico`、`avif`、`tif/tiff`、`svg` 可以登记并嵌入产物，但不会直接送入视觉模型；需要先转换为上述视觉格式。
- 如果当前模型被配置为纯文本模型，工具会明确返回“未执行视觉检查”；此时不要声称已经看过或验证过图片，应提示用户切换到支持视觉的模型。图片仍可通过 `asset://` 引用嵌入产物。

## read_pdf

读取工作区里的 PDF 文件并按页提取嵌入文本。**最佳效果是文字型 PDF**；扫描件（无文本层）可能返回空页面。
...
**注意事项**：
- 单个 PDF 上限 100 MB。
- 表格、二进制图表、嵌入字体在文本抽取中可能丢失排版。
- 当前没有把 PDF 页面渲染为图片的工具，也没有 OCR；`read_image` 不能直接读取 PDF。若任务依赖扫描页或版式视觉信息，必须让用户提供对应页面的 PNG/JPEG/WEBP/GIF 截图，再用 `read_image` 检查。没有截图时应明确说明未完成视觉验证，不能根据空文本猜测页面内容。
