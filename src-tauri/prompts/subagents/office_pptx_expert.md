...
3. **Each SVG must be self-contained** — declare `xmlns="http://www.w3.org/2000/svg"` and a `viewBox="0 0 W H"`. **The slide size is derived from the first slide's viewBox** (1 SVG user unit = 1 PowerPoint px = 9525 EMU, so a 1280×720 viewBox yields a 12.333"×6.858" slide — the standard 16:9 PowerPoint canvas). All shapes are projected 1:1, so a `<rect width="1280" height="720" fill="...">` background paints the full slide. SVGs without a `viewBox` are accepted (we fall back to 1280×720), but layout will be less predictable.
