...
///   4. <c>Releases:StorageDir</c> — uploaded installer binaries served from
///                                  <c>/releases-files/{file}</c> (no auth; the
///                                  upload pipeline writes directly here).
/// </summary>
public static class SpaFallbackExtensions
{
    public static WebApplication MapAdminSpa(this WebApplication app)
    {
        ...
        var releasesRoot = app.Configuration["Releases:StorageDir"] ?? "/var/lib/inkuo/releases";

        // 1. Installer binaries — must be registered BEFORE the generic static
        //    middleware so /releases-files/* never falls through to the SPA
        //    fallback. These are public (no auth) by design.
        if (Directory.Exists(releasesRoot))
        {
            app.UseStaticFiles(new StaticFileOptions
            {
                FileProvider = new Microsoft.Extensions.FileProviders.PhysicalFileProvider(releasesRoot),
                RequestPath = "/releases-files",
                ServeUnknownFileTypes = true,
                DefaultContentType = "application/octet-stream",
            });
        }

        // 2. Generic static files (favicon, marketing assets, etc.).
        app.UseStaticFiles(new StaticFileOptions
        {
        ...
        // 3. Admin SPA: any GET under /admin that didn't match a static file
        ...
        // 4. Marketing site fallback. The root must serve the marketing
        ...
        app.MapFallback(async ctx =>
        {
            ...
            if (!File.Exists(indexPath))
            {
                ...
                await ctx.Response.WriteAsync(
                    ...
                    "<p>Build it with <code>pnpm --dir cloud-server/marketing-frontend build</code> " +
